...
UNSD provides datasets open to the public on various topics related to international development, economic metrics and trade. For this investigation data on total municipal waste generation and GDP were needed. Total municipal waste generation was available for the years 1990-2016, aggregated by year and country. [cit] GPD per capita data was available for the years 1970-2018, aggregated by year and country. [cit] Data for the years 1989-2018 were downloaded for this investigation. To allow fair comparison between waste generation and GDP, the waste data was processed to represent municipal waste generated per capita. To calculate this we needed population data for each of the corresponding countries and years. Population data was available from UNSD for the years 1948-2020, aggregated by year and country. Data for the years 1990-2020 were downloaded for this investigation. All data was downloaded as Comma Separated Values (CSV) format. Municipal waste per capita was calculate by dividing the total municipal waste by the total population, for each year and country in the dataset.

## Data variability
```{r plotsdataprep}
```
Each of the datasets don't cover exactly the same set of countries or years. To view this variability, a summary of each of the datasets is presented in Table {ref}.

<!-- Table -->

\*\* TABLE SHOWING SUMMARIES \*\* \|\|Number of countries\|Years\|Number of values\|Max\|Min\|Quartiles?\| <!-- ```{r table_summary, echo=FALSE, results="asis"} --> <!-- knitr::kable(data_sum, caption="Dataset summary statistics") --> <!-- ``` -->

It shows that ...

For each of the datasets (waste, GDP, population) we can plot the distributions of the values and geographical spreads

```{r wasteplot, include=TRUE, fig.cap="Distribution of municipal waste values and countries for which the data is relevant."}
```



```{r gdpplot, include=TRUE, fig.cap="Distribution of GDP per capita values and countries for which the data is relevant."}
```

```{r gdpplot, include=TRUE, fig.cap="Distribution of population values and countries for which the data is relevant."}
```

We can see from... - Log-normal distribution - Not all same countries covered

## Data merging

This investigation is trying to understand the variation of municipal waste generation with GDP and possibly population. Therefore we are treating municipal waste generation as a response variable, and GDP and population as explanatory variables. We can merge our datasets together, but as we saw in above {ref} each dataset does not cover the same number of countries. Given municipal waste is our response variable, it was chosen to keep all the data from this dataset and merge on only matching countries and years from the other two datasets.
